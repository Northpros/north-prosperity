...
function WithdrawalTab({plan, results, T}) {
  const ea=plan.divestAssets.filter(a=>a.enabled&&a.shares>0&&a.pricePerShare>0);
  if(!results.length||!ea.length) return <div style={{width:"100%"}}><Card title="Withdrawal Plan" T={T}><Empty T={T} msg="Enable divest assets to see withdrawal schedule."/></Card></div>;
  ...
  return <div style={{width:"100%"}}><Card title="Annual Withdrawal Plan" T={T}>
    <Hint T={T}>Shares to sell each year and income generated from each divest asset.</Hint>
    <div style={{overflowX:"auto",overflowY:"auto",maxHeight:"75vh",width:"100%"}}>
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        ...
        <tbody>{results.map((r,i)=>{const tw=r.assets.reduce((t,a)=>t+a.withdrawal,0);return<tr key={i} style={{background:i%2?T.rowAlt:"transparent",borderBottom:`1px solid ${T.border}`}}>
          <td style={{...td,position:"sticky",left:0,zIndex:1,background:i%2?T.rowAlt:T.card,fontWeight:600,textAlign:"left"}}>{r.year}</td>
