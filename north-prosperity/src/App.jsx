...
const toBase = (v,cur,base,rates) => { if(!rates||cur===base)return v; const usdV=cur==="USD"?v:(rates[cur]?v/rates[cur]:v); return base==="USD"?usdV:(rates[base]?usdV*rates[base]:usdV); };
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:12,width:"100%"}}>
    <Card title="Planning Parameters" T={T}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:10}}>
        ...
        {fxRate&&<div style={{fontSize:9,color:T.textDim,fontFamily:FONT_MONO,marginTop:3}}>Live rates loaded ✓</div>}
  ...
  return<div style={{display:"flex",flexDirection:"column",gap:12,width:"100%"}}>
    ...
    <Card title="Big Ticket Calculator" T={T} action={plan.bigTicketStocks.length<10?()=>update(d=>{d.bigTicketStocks.push({id:mkId(),ticker:"",shares:0,price:0,enabled:false});return d;}):null} actionLabel="+ Add">
      ...
      {total>0&&<div style={{background:T.summaryBg,border:`1px solid ${T.gold}20`,borderRadius:10,padding:18,marginTop:10,textAlign:"center"}}>
        ...
        <div style={{fontFamily:FONT_DISPLAY,fontSize:26,fontWeight:700,color:T.gold,marginBottom:8}}>{fmt(totalInBase,base)}</div>
...
function CurrencyTag({currency,onChange,base,T}){
  const isForeign=(currency||base)!==base;
  return<div style={{display:"flex",flexDirection:"column",alignItems:"center",minWidth:54}}>
    ...
    <select value={currency||base} onChange={e=>onChange(e.target.value)} style={{
      padding:"2px 4px",fontSize:9,fontWeight:600,background:isForeign?`${T.gold}20`:T.inputBg,
      color:isForeign?T.gold:T.text,border:`1px solid ${isForeign?T.gold:T.border2}`,
