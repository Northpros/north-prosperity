    ...
    <Card title="Tax Deferred Sources of Income" badge="RRSP, TFSA, RRIF, 401(k), IRA, ISA, SIPP, Super, KiwiSaver, Pillar 3a, Afore" T={T}
      ...
      {plan.investmentIncome.map((s,i)=><div key={s.id}>
        <ItemRow enabled={s.enabled} T={T} onToggle={()=>update(d=>{d.investmentIncome[i].enabled=!d.investmentIncome[i].enabled;return d;})} onRemove={()=>update(d=>{d.investmentIncome.splice(i,1);return d;})}>
          ...
          <MF label="Price" value={s.pricePerShare} type="number" w="0.38fr" onChange={v=>update(d=>{d.investmentIncome[i].pricePerShare=+v||0;return d;})} T={T}/>
    ...
    <Card title="Investment Assets to Divest" badge="Max 20" T={T}
      ...
      {plan.divestAssets.map((a,i)=><div key={a.id}>
        <ItemRow enabled={a.enabled} T={T} onToggle={()=>update(d=>{d.divestAssets[i].enabled=!d.divestAssets[i].enabled;return d;})} onRemove={()=>update(d=>{d.divestAssets.splice(i,1);return d;})}>
          ...
          <MF label="Shares" value={a.shares} type="number" w="0.34fr" fixed onChange={v=>update(d=>{d.divestAssets[i].shares=+v||0;return d;})} T={T}/>
          <MF label="Price" value={a.pricePerShare} type="number" w="0.38fr" fixed onChange={v=>update(d=>{d.divestAssets[i].pricePerShare=+v||0;return d;})} T={T}/>
    ...
    <Card title="Fixed Assets (Non-Income)" badge="Real Estate, Precious Metals, Collectibles, Hard Assets" T={T}
      ...
      {plan.fixedAssets.map((a,i)=><div key={a.id}>
        <ItemRow enabled={a.enabled} T={T} onToggle={()=>update(d=>{d.fixedAssets[i].enabled=!d.fixedAssets[i].enabled;return d;})} onRemove={()=>update(d=>{d.fixedAssets.splice(i,1);return d;})}>
          ...
          <MF label="Units" value={a.shares} type="number" w="0.38fr" fixed onChange={v=>update(d=>{d.fixedAssets[i].shares=+v||0;return d;})} T={T}/>
          <MF label="Price" value={a.pricePerShare} type="number" w="0.6fr" fixed onChange={v=>update(d=>{d.fixedAssets[i].pricePerShare=+v||0;return d;})} T={T}/>
...
function MF({label,value,onChange,type="text",step,w="1fr",fixed=false,T}){return<div style={{minWidth:30,flex:w,flexShrink:fixed?0:1}}><label style={{fontSize:9,color:T.label,fontWeight:600,textTransform:"uppercase",letterSpacing:0.3,display:"block",marginBottom:2,fontFamily:FONT_LABEL}}>{label}</label>
