...
function PlanningTab({plan, update, T, baseCurrency="USD", fxRate=null, fxError=false}) {
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:12,width:"100%"}}>
    ...
    <Card title="Fixed Sources of Income" badge="Pension, CPP, OAS, Social Security, GIS, Annuity, DPSP" T={T}
      ...
      {plan.fixedIncome.map((s,i)=><ItemRow key={s.id} enabled={s.enabled} T={T} onToggle={()=>update(d=>{d.fixedIncome[i].enabled=!d.fixedIncome[i].enabled;return d;})} onRemove={()=>update(d=>{d.fixedIncome.splice(i,1);return d;})}>
        ...
        <MF label="Annual $" value={s.amount} type="number" w="0.4fr" onChange={v=>update(d=>{d.fixedIncome[i].amount=+v||0;return d;})} T={T}/>
        <MF label="Start Year" value={s.startYear} type="number" w="0.25fr" onChange={v=>update(d=>{d.fixedIncome[i].startYear=+v||2030;return d;})} T={T}/>
        <MF label="Index%" value={s.indexing} type="number" step="0.5" w="0.22fr" onChange={v=>update(d=>{d.fixedIncome[i].indexing=+v||0;return d;})} T={T}/>
    ...
    <Card title="Tax Deferred Sources of Income" badge="RRSP, TFSA, RRIF, 401(k), IRA, ISA, SIPP, Super, KiwiSaver, Pillar 3a, Afore" T={T}
      ...
      {plan.investmentIncome.map((s,i)=><div key={s.id}>
        <ItemRow enabled={s.enabled} T={T} onToggle={()=>update(d=>{d.investmentIncome[i].enabled=!d.investmentIncome[i].enabled;return d;})} onRemove={()=>update(d=>{d.investmentIncome.splice(i,1);return d;})}>
          ...
          <MF label="Shares" value={s.shares} type="number" w="0.25fr" onChange={v=>update(d=>{d.investmentIncome[i].shares=+v||0;return d;})} T={T}/>
          <MF label="Price" value={s.pricePerShare} type="number" w="0.4fr" onChange={v=>update(d=>{d.investmentIncome[i].pricePerShare=+v||0;return d;})} T={T}/>
          <MF label="CAGR%" value={s.cagr} type="number" step="0.5" w="0.22fr" onChange={v=>update(d=>{d.investmentIncome[i].cagr=+v||0;return d;})} T={T}/>
          <MF label="Yr 1-5 ↓%" value={s.cagrDecline1!==undefined?s.cagrDecline1:(s.cagrDecline||0.3)} type="number" step="0.1" w="0.22fr" onChange={v=>update(d=>{d.investmentIncome[i].cagrDecline1=+v||0;return d;})} T={T}/>
          <MF label="Yr 6-20 ↓%" value={s.cagrDecline2!==undefined?s.cagrDecline2:((s.cagrDecline||0.3)*0.6)} type="number" step="0.1" w="0.22fr" onChange={v=>update(d=>{d.investmentIncome[i].cagrDecline2=+v||0;return d;})} T={T}/>
          <MF label="Yr 21+ ↓%" value={s.cagrDecline3!==undefined?s.cagrDecline3:((s.cagrDecline||0.3)*0.3)} type="number" step="0.1" w="0.22fr" onChange={v=>update(d=>{d.investmentIncome[i].cagrDecline3=+v||0;return d;})} T={T}/>
          <MF label="Div%" value={s.dividendPercent} type="number" step="0.5" w="0.22fr" onChange={v=>update(d=>{d.investmentIncome[i].dividendPercent=+v||0;return d;})} T={T}/>
    ...
    <Card title="Other Sources of Income" badge="Business, Rental" T={T}
      ...
      {plan.otherIncome.map((s,i)=><React.Fragment key={s.id}><ItemRow enabled={s.enabled} T={T} onToggle={()=>update(d=>{d.otherIncome[i].enabled=!d.otherIncome[i].enabled;return d;})} onRemove={()=>update(d=>{d.otherIncome.splice(i,1);return d;})}>
        ...
        <MF label="Units" value={s.shares} type="number" w="0.25fr" onChange={v=>update(d=>{d.otherIncome[i].shares=+v||0;return d;})} T={T}/>
        <MF label="Price" value={s.pricePerShare} type="number" w="0.4fr" onChange={v=>update(d=>{d.otherIncome[i].pricePerShare=+v||0;return d;})} T={T}/>
        <MF label="CAGR%" value={s.cagr} type="number" step="0.5" w="0.22fr" onChange={v=>update(d=>{d.otherIncome[i].cagr=+v||0;return d;})} T={T}/>
        <MF label="Decl%" value={s.cagrDecline} type="number" step="0.1" w="0.22fr" onChange={v=>update(d=>{d.otherIncome[i].cagrDecline=+v||0;return d;})} T={T}/>
        <MF label="Annual$" value={s.annualIncome} type="number" w="0.4fr" onChange={v=>update(d=>{d.otherIncome[i].annualIncome=+v||0;return d;})} T={T}/>
    ...
    <Card title="Investment Assets to Divest" badge="Max 20" T={T}
      ...
      {plan.divestAssets.map((a,i)=><div key={a.id}>
        <ItemRow enabled={a.enabled} T={T} onToggle={()=>update(d=>{d.divestAssets[i].enabled=!d.divestAssets[i].enabled;return d;})} onRemove={()=>update(d=>{d.divestAssets.splice(i,1);return d;})}>
          ...
          <MF label="Shares" value={a.shares} type="number" w="0.25fr" onChange={v=>update(d=>{d.divestAssets[i].shares=+v||0;return d;})} T={T}/>
          <MF label="Price" value={a.pricePerShare} type="number" w="0.4fr" onChange={v=>update(d=>{d.divestAssets[i].pricePerShare=+v||0;return d;})} T={T}/>
          <MF label="CAGR%" value={a.cagr} type="number" step="1" w="0.22fr" onChange={v=>update(d=>{d.divestAssets[i].cagr=+v||0;return d;})} T={T}/>
          <MF label="Yr 1-5 ↓%" value={a.cagrDecline1} type="number" step="0.1" w="0.22fr" onChange={v=>update(d=>{d.divestAssets[i].cagrDecline1=+v||0;return d;})} T={T}/>
          <MF label="Yr 6-20 ↓%" value={a.cagrDecline2} type="number" step="0.1" w="0.22fr" onChange={v=>update(d=>{d.divestAssets[i].cagrDecline2=+v||0;return d;})} T={T}/>
          <MF label="Yr 21+ ↓%" value={a.cagrDecline3} type="number" step="0.1" w="0.22fr" onChange={v=>update(d=>{d.divestAssets[i].cagrDecline3=+v||0;return d;})} T={T}/>
          <MF label="Div%" value={a.dividendPercent||0} type="number" step="0.5" w="0.22fr" onChange={v=>update(d=>{d.divestAssets[i].dividendPercent=+v||0;return d;})} T={T}/>
    ...
    <Card title="Fixed Assets (Non-Income)" badge="Real Estate, Precious Metals, Collectibles, Hard Assets" T={T}
      ...
      {plan.fixedAssets.map((a,i)=><div key={a.id}>
        <ItemRow enabled={a.enabled} T={T} onToggle={()=>update(d=>{d.fixedAssets[i].enabled=!d.fixedAssets[i].enabled;return d;})} onRemove={()=>update(d=>{d.fixedAssets.splice(i,1);return d;})}>
          ...
          <MF label="Units" value={a.shares} type="number" w="0.25fr" onChange={v=>update(d=>{d.fixedAssets[i].shares=+v||0;return d;})} T={T}/>
          <MF label="Price" value={a.pricePerShare} type="number" w="0.4fr" onChange={v=>update(d=>{d.fixedAssets[i].pricePerShare=+v||0;return d;})} T={T}/>
          <MF label="CAGR%" value={a.cagr} type="number" step="0.5" w="0.22fr" onChange={v=>update(d=>{d.fixedAssets[i].cagr=+v||0;return d;})} T={T}/>
          <MF label="Yr 1-5 ↓%" value={a.cagrDecline1!==undefined?a.cagrDecline1:(a.cagrDecline||0.1)} type="number" step="0.1" w="0.22fr" onChange={v=>update(d=>{d.fixedAssets[i].cagrDecline1=+v||0;return d;})} T={T}/>
          <MF label="Yr 6-20 ↓%" value={a.cagrDecline2!==undefined?a.cagrDecline2:((a.cagrDecline||0.1)*0.5)} type="number" step="0.1" w="0.22fr" onChange={v=>update(d=>{d.fixedAssets[i].cagrDecline2=+v||0;return d;})} T={T}/>
          <MF label="Yr 21+ ↓%" value={a.cagrDecline3!==undefined?a.cagrDecline3:((a.cagrDecline||0.1)*0.2)} type="number" step="0.1" w="0.22fr" onChange={v=>update(d=>{d.fixedAssets[i].cagrDecline3=+v||0;return d;})} T={T}/>
    ...
    <div style={{overflowX:"auto",overflowY:"auto",maxHeight:"75vh",width:"100%"}}>
      <table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
        ...
        <tbody>{results.map((r,i)=><tr key={i} style={{background:i%2?T.rowAlt:"transparent",borderBottom:`1px solid ${T.border}`}}>
          ...
          <td style={td}>{r.age}</td><td style={{...td,fontWeight:700,color:T.gold}}>{fmt(r.totalIncome,bc)}</td><td style={{...td,fontWeight:700,color:T.green}}>{fmtK(r.totalValue,bc)}</td>
      ...
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        ...
        <tbody>{results.map((r,i)=>{
          ...
          return<tr key={i} style={{background:i%2?T.rowAlt:"transparent",borderBottom:`1px solid ${T.border}`}}>
            <td style={{...td,position:"sticky",left:0,zIndex:1,background:i%2?T.rowAlt:T.card,fontWeight:600,textAlign:"left"}}>{r.year}</td>
            <td style={td}>{r.age}</td><td style={{...td,fontWeight:700,color:T.gold}}>{fmt(tw,bc)}</td>
            {r.assets.map((a,j)=><React.Fragment key={j}><td style={{...td,color:T.accent}}>{fmtN(a.sharesSold,4)}</td><td style={{...td,color:T.green}}>{fmt(a.withdrawal,bc)}</td><td style={{...td,color:T.textDim}}>{fmtN(a.shares,2)}</td></React.Fragment>)}
            {(r.investmentIncomeSources||[]).map((s,j)=><React.Fragment key={j}><td style={{...td,color:T.cyan}}>{fmtN(s.sharesSold,4)}</td><td style={{...td,color:T.green}}>{fmt(s.dividendIncome,bc)}</td><td style={{...td,color:T.textDim}}>{fmtN(s.shares,2)}</td></React.Fragment>)}
          </tr>;})}</tbody>
      </table>
    </div>
  </Card></div>;
  ...
  return<div style={{display:"flex",flexDirection:"column",gap:12,width:"100%"}}>
    ...
    <Card title="Big Ticket Calculator" T={T} action={plan.bigTicketStocks.length<10?()=>update(d=>{d.bigTicketStocks.push({id:mkId(),ticker:"",shares:0,price:0,enabled:false});return d;}):null} actionLabel="+ Add">
      ...
      <div style={{marginTop:10}}>{plan.bigTicketStocks.map((s,i)=><React.Fragment key={s.id}><ItemRow enabled={s.enabled} T={T} onToggle={()=>update(d=>{d.bigTicketStocks[i].enabled=!d.bigTicketStocks[i].enabled;return d;})} onRemove={()=>update(d=>{d.bigTicketStocks.splice(i,1);return d;})}>
        ...
        <MF label="Shares" value={s.shares} type="number" w="0.25fr" onChange={v=>update(d=>{d.bigTicketStocks[i].shares=+v||0;return d;})} T={T}/>
        <MF label="Price" value={s.price} type="number" w="0.4fr" onChange={v=>update(d=>{d.bigTicketStocks[i].price=+v||0;return d;})} T={T}/>
...
function MF({label,value,onChange,type="text",step,w="1fr",lock=false,px,T}){return<div style={{minWidth:30,flex:px?`0 0 ${px}px`:lock?"0 0 auto":w,width:px?px:undefined}}><label style={{fontSize:9,color:T.label,fontWeight:600,textTransform:"uppercase",letterSpacing:0.3,display:"block",marginBottom:2,fontFamily:FONT_LABEL}}>{label}</label>
